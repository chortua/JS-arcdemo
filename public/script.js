...
let ip_address = "192.168.1.71" /*"192.168.1.71";  ip of Server (localhost:)*/
...
let groupzone = { 
    groupPick1:["0-CargoP-1","0-CargoP-2","0-CargoP-3"],
    groupPick2:["0-CargoP-4","0-CargoP-5","0-CargoP-6"],
    groupPick3:["0-CargoP-7","0-CargoP-8","0-CargoP-9"],
    groupDrop1:["0-CargoDrop-3","0-CargoDrop-2","0-CargoDrop-1"],
    groupDrop2:["0-CargoDrop-6","0-CargoDrop-5","0-CargoDrop-4"],
    groupDrop3:["0-CargoDrop-9","0-CargoDrop-8","0-CargoDrop-7"],
}
...
let points_info = {
    coordinates: [
        ...
        { name: "0-A-Charger-HP", x: (-2.9), y: (0 - 0.5), action: "pickup" },
        ...
        { name: "0-CargoP-1", x: -2.008, y: -5.863, action: "pickup" },
        { name: "0-CargoP-2", x: -0.608, y: -5.863, action: "pickup" },
        { name: "0-CargoP-3", x: 0.792, y: -5.863, action: "pickup" },
        { name: "0-CargoP-4", x: -2.008, y: -7.049, action: "pickup" },
        { name: "0-CargoP-5", x: -0.608, y: -7.049, action: "pickup" },
        { name: "0-CargoP-6", x: 0.792, y: -7.049, action: "pickup" },
        { name: "0-CargoP-7", x: -2.008, y: -8.275, action: "pickup" },
        { name: "0-CargoP-8", x: -0.608, y: -8.275, action: "pickup" },
        { name: "0-CargoP-9", x: 0.792, y: -8.275, action: "pickup" },
        {  name: "groupDrop1", x: 12.9, y: -7.0, action: "drop"},
        {  name: "groupDrop2", x: 14.1, y: -7.0, action: "drop"},
        {  name: "groupDrop3", x: 15.3, y: -7.0, action: "drop"},
        {  name: "groupPick1", x: 9.5, y: -16, action: "pickup"},
        {  name: "groupPick2", x: 9.5, y: -17.17, action: "pickup"},
        {  name: "groupPick3", x: 9.5, y: -18.4, action: "pickup"},
...
function create_points(info) {
    for (let i = 0, len = info.length; i < len; i++) {
        ...
        if ( info[i].action == "pickup"){
            pickup_locations.push(info[i].name);
        }
    }
}
...
function reply_click(clicked_id) {
    ...
    if (counter_mission === 0) {
       if(pickup_locations.includes(clicked_id)){
...
async function create_mission() {
    ...
    if(from == "groupPick1" || from == "groupPick2" || from == "groupPick3" )
    { for (let i = 0, len = 3 ; i < len; i++){
        
        console.log(groupzone["groupPick1"][i])
        ...
        const api_create_missions = await fetch(url, {
            ...
            body: JSON.stringify({
                missionrequest: {
                    ...
                    missiontype: "7",
                    fromnode: groupzone[from][i],
                    tonode: groupzone[to][i],
                    cardinality: "1",
                    priority: 2,
                    parameters: {
...
async function track_missions() {
    ...
    for (let i = 0, len = data_missions["payload"]["missions"].length; i < len; i++){       
        if(data_missions["payload"]["missions"][i]["navigationstate"] == "1"){
            ...
            mission_id.push(data_missions["payload"]["missions"][i]["missionid"]);
            document.getElementById("mission_status").innerHTML += `<h4>${from}&nbsp; &nbsp;  >>>  &nbsp; &nbsp; &nbsp;  ${to}<br></h4>`;
    ...
    for (let i = 0, len = data_missions["payload"]["missions"].length; i < len; i++){       
        if(data_missions["payload"]["missions"][i]["navigationstate"] == "3"){
            running_mission = data_missions["payload"]["missions"][i]["missionid"];
            let from = data_missions["payload"]["missions"][i]["fromnode"];
            let to = data_missions["payload"]["missions"][i]["tonode"];
            
            /*document.getElementById(from).style.backgroundColor = "#da09bb";
            document.getElementById(to).style.backgroundColor = "#da09bb";*/
...
async function delete_missions() {
    var answer = window.confirm("保留中のミッションは全て削除され、車両は停止します。マニュアルで充電器まで持っていき、車両の抜き取り、挿入を行い、再度ミッションを作成して下さい。")
    if (answer) {
        const response = await fetch("http://" + ip_address + ":8081/wms/monitor/session/login?username=admin&pwd=123456");
        const data = await response.json();
        for (let i = 0, len = mission_id.length; i < len; i++) {
            let url = `http://${ip_address}:8081/wms/rest/missions/${mission_id[i]}?&sessiontoken=${data["payload"]["sessiontoken"]}`
            const api_delete_missions = await fetch(url, {
                method: 'DELETE',
            })

        const deleted_missions = await api_delete_missions.json()
    }
}
    else {
        //some code
...
async function delete_running_mission() {
    var answer = window.confirm("この操作により、すべてのミッションが削除され、AVGは動かなくなります。AVG を再起動するには、充電器まで持っていき、取り出し、再度挿入して、新しいミッションを作成してください.");
        if (answer) {
                const response = await fetch("http://" + ip_address + ":8081/wms/monitor/session/login?username=admin&pwd=123456");
                const data = await response.json();
                let url = `http://${ip_address}:8081/wms/rest/missions/${running_mission}?&sessiontoken=${data["payload"]["sessiontoken"]}`
                const api_delete_missions = await fetch(url, {
                    method: 'DELETE',
                })
                const deleted_missions = await api_delete_missions.json()
